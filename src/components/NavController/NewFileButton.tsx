...
import { removeUserTemplate, getUserTemplateIds, removeTemplate } from '@/utils/localStorage';
...
const NewFileButton: FC<NewFileButtonProps> = props => {
    ...
    const newIcon = (
        <a className={styles.newFileButton} onClick={handleClickNew}>
    ...
    return menuNewDropDown ? (
        <Dropdown overlay={menuNewDropDown} placement="bottomCenter">
            {newIcon}
        </Dropdown>
    ) : (
        newIcon
